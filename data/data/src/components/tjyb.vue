<template>
  <div class="container_01">
    ...
    <div class="header left">
        ...
        <div class="right nav text_right">
          <ul>
            ...
            <li><i class="nav_10"></i><router-link to="/xjcx">星级查询</router-link></li>
          </ul>
        </div>
    </div>
    <!--内容部分-->
    <div class="con left">
        ...
        <div class="div_any">
            <div class="div_any002" style="width:1860px">
                <div class="div_any_child div_height">
                  <div class="div_any_title"><img src="../../static/images/title_1.png">8890诉求统计&nbsp;&nbsp;&nbsp;&nbsp;日期：{{start}}  -  {{end}}
                    <button type="button" name="button" class="downexcel_02" @click="exportWord" style="display:inline">下载</button>
                  </div>
        ...
        <div class="div_any">
            <div class="left div_any001" style = "width:1221px">
                <div class="div_any_child">
                  <div class="div_any_title"><img src="../../static/images/title_1.png">七大板块诉求汇总</div>
                  <div class="div_01_child_child_01" v-for="(qida,index) in qidas.data" style="width:140px;">
                    <span></span>
                    <span></span>
                    <span></span>
                    <span></span>
            ...
            <!--<div class="" v-for="(val,k) in bTitle">
              <div class="right div_any001" style="width:589px">
                  <div class="div_any_child">
...
export default {
  ...
  mounted(){
    ...
    //七大板块
    this.get(host.url+'/qida').then(res=>{
      this.qidas = res.data[0]
    })
    //紧急诉求分布
  ...
  methods:{
    ...
    search:function(){
      ...
      //七大板块
      this.get(host.url+'/qida',{start:this.start,end:this.end}).then(res=>{
        this.qidas = res.data[0]
      })
      //诉求前10类热点问题
      //柱形图
      ...
      this.get(host.url+'/onetwo',{start:this.start,end:this.end}).then(res=>{
        if(res.data == ''){
          this.drawBar_color('echart_0','暂无数据','诉求热点',0);
          this.drawBar_color('echart_1','暂无数据','诉求热点',0);
          this.drawBar_color('echart_2','暂无数据','诉求热点',0);
          this.drawBar_color('echart_3','暂无数据','诉求热点',0);
          this.drawBar_color('echart_4','暂无数据','诉求热点',0);
          this.drawBar_color('echart_5','暂无数据','诉求热点',0);
          this.drawBar_color('echart_6','暂无数据','诉求热点',0);
          this.drawBar_color('echart_7','暂无数据','诉求热点',0);
          this.drawBar_color('echart_8','暂无数据','诉求热点',0);
          this.drawBar_color('echart_9','暂无数据','诉求热点',0);
          this.drawBar_color('Chart01','暂无数据','诉求热点',0);
        }else{
          this.drawBar_color('Chart01',res.data.data,'诉求热点',res.data.num);
          this.big_data = res.data
          //console.log('1111',this.big_data.leveltwo)
...
*/
        ...
        this.axios.get(host.url+'/ReportWord?rtime='+this.start+','+this.end)
          .then(function(res){
          ...
          .catch(function(error){
            ...
            if(error.response.status == 500){
              $('.load_img').hide();
              $('.cover_word').show();
              $('.cover_word').html('数据过少，无法生成报表，请重新选择查询日期！！！') ;
              $('#cover').css("display","block");
              setTimeout(function(){
                $('.load_img').hide();
                $('#cover').hide();
